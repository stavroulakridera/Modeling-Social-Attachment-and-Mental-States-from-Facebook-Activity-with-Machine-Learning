...
def get_comment_frequency(comments_json_file, df):
    ...
    try:
        author = comments[0]["data"][0]["comment"][
            "author"
        ]  # name of the person in concern. (eg: Harini)
...
def cleaning(path):
    ...
    posts_in_data = posts_df["data"].apply(pd.Series)[0].apply(pd.Series)["post"]
...
def get_post_metrics(df, post_file):
    ...
    for friend in df.name:
        ...
        if df.loc[ind, "post frequency"] != 0:
            ...
            friend_post["empath"] = friend_post["post"].apply(
                lambda x: sum(lexicon.analyze(x, categories=eleven_categories).values())
            )
        ...
        try:
            ...
            if len(data["participants"]) == 2:
                ...
                while os.path.exists(
                    os.path.join(inbox_path, chat, f"message_{i}.json")
                ):
                    message_file = open(
                        os.path.join(inbox_path, chat, f"message_{i}.json")
                    )
                ...
                first_timestamp = ((data["messages"])[first_message_index])[
                    "timestamp_ms"
                ] / 1000
    ...
    if not (isinstance(hash, float) and math.isnan(hash)):
        ...
        for message in chat_data["messages"]:
            if "content" in message:
                chat_to_analyze.append(
                    sentiment_analyzer_scores(message["content"])["pos"]
                )
...
def linear_regression(df):
    ...
    y = (
        (-0.76) * df_normal["days since last communication"]
        + 0.111 * df_normal["empath"]
        + 0.135 * df_normal["vader positivity"]
        + 0.299 * df_normal["post frequency"]
        + 0.299 * df_normal["number of messages"]
        + 0.299 * df_normal["comment frequency"]
    )
    y_max = max(y)
    y_min = min(y)
...
def normalize(df):
    ...
    for feature_name in df.columns:
        if np.std(df[feature_name]) == 0:
            ...
            result[feature_name] = (df[feature_name] - np.mean(df[feature_name])) / (
                np.std(df[feature_name])
            )
    return result
...
def update_friend_tone(inbox_path, friend_analyze):
    ...
    if (
        len(ind) == 1
    ):  # checks if the friend's name is in the dataframe and if it is unique
        row_num = ind[0]
        hash = df.at[row_num, "chat hash"]
        ...
        if len(paragraph["text"]) > 0:
            chat_results_tone = ta.tone(paragraph).get_result()["document_tone"][
                "tones"
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path",
        help="path to facebook dataset",
        nargs="?",
        default="facebook-sampledataset",
    )
    ...
    df["days since being friends"] = (
        df["days since being friends"].astype("timedelta64[h]") / 24
    )
    dt_now = datetime.utcnow()
    df["days since last communication"] = df["date of last message"].apply(
        lambda ts: dt_now - ts
    )
    df["days since last communication"] = (
        df["days since last communication"].astype("timedelta64[h]") / 24
    )
    df["days since last communication"] = df["days since last communication"].fillna(
        df["days since being friends"]
    )
    ...
    df = df.sort_values(
        by=["tie strength"], ascending=False
    )  # sort by the tie strength
...
def attachment_strength_original(days_since_last_comm, words_intimacy, pos_emotions, wall_posts, messages, comments):
    """Compute Attachment Strength (original equation)."""
    return (
        -0.76 * days_since_last_comm
        + 0.111 * words_intimacy
        + 0.135 * pos_emotions
        + 0.299 * wall_posts
        + 0.299 * messages
        + 0.299 * comments
...
def attachment_strength_revised(days_since_last_comm, words_intimacy, pos_emotions, wall_posts, messages, comments):
    """Compute Attachment Strength (revised equation)."""
    return (
        -0.85 * days_since_last_comm
        + 0.25 * words_intimacy
        + 0.25 * pos_emotions
        + 0.27 * wall_posts
        + 0.27 * messages
